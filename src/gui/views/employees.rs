...
use crate::gui::{Material3Colors, material_button};
use crate::gui::app::{PersonnelApp, ConfirmAction, EditDialog};
...
impl PersonnelApp {
    pub fn show_employees(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            ui.label(RichText::new("Employees").size(28.0).color(colors.on_surface).strong());
        ...
        ui.label(RichText::new("All Employees").size(16.0).color(colors.on_surface_variant));
        ui.add_space(12.0);
            ...
            Frame::none()
                ...
                .show(ui, |ui| {
                    ui.centered_and_justified(|ui| {
                        ui.label(RichText::new("No employees yet").color(colors.on_surface_variant));
                    });
                });
        }
    ...
    fn render_employee_card(&mut self, ui: &mut Ui, colors: &Material3Colors, emp: &Employee, depts: &[Department], all_emps: &[Employee], grades: &[SalaryGrade]) {
        Frame::none()
            .fill(colors.surface_variant)
            .stroke(Stroke::NONE)
            .rounding(Rounding::same(12.0))
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ui.label(RichText::new(format!("{} {}", emp.first_name, emp.last_name)).size(16.0).strong().color(colors.on_surface));
                        ui.add_space(4.0);
                        ui.label(RichText::new(&emp.email).size(13.0).color(colors.on_surface_variant));
                        ui.label(RichText::new(format!("Role: {}", emp.role)).size(13.0).color(colors.on_surface_variant));
                        let dept_name = emp.department_id.as_ref()
                            .and_then(|id| depts.iter().find(|d| &d.id == id))
                            .map(|d| d.name.clone())
                            .unwrap_or_else(|| "No department".to_string());
                        ui.label(RichText::new(format!("Dept: {}", dept_name)).size(13.0).color(colors.on_surface_variant));
                        ...
                        ui.label(RichText::new(format!("Grade: {}", grade_info)).size(13.0).color(colors.on_surface_variant));
                    });
                    
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        let delete_btn = Button::new(RichText::new("🗑 Delete").size(12.0).color(Color32::WHITE))
                        ...
                        ui.add_space(8.0);
                        let edit_btn = Button::new(RichText::new("✏ Edit").size(12.0).color(colors.on_primary))
                            .fill(colors.primary)
                            .stroke(Stroke::NONE)
                            .rounding(Rounding::same(8.0))
                        ...
                        if ui.add(edit_btn).clicked() {
                            ...
                            self.emp_salary_grade_id = emp.salary_grade_id.clone().unwrap_or_default();
                            self.edit_dialog = Some(EditDialog::EditEmployee { id: emp.id.clone() });
                        }
                    });
                });
            });
