...
impl PersonnelApp {
    pub fn show_salary_grades(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            ui.label(RichText::new("Salary Grades").size(28.0).color(colors.on_surface).strong());
        ...
        ui.label(RichText::new("All Salary Grades").size(16.0).color(colors.on_surface_variant));
        ui.add_space(12.0);
            ...
            Frame::none()
                ...
                .show(ui, |ui| {
                    ui.centered_and_justified(|ui| {
                        ui.label(RichText::new("No salary grades yet").color(colors.on_surface_variant));
                    });
                });
        }
    ...
    fn render_grade_card(&mut self, ui: &mut Ui, colors: &Material3Colors, grade: &SalaryGrade, employee_count: usize) {
        Frame::none()
            .fill(colors.surface_variant)
            .stroke(Stroke::NONE)
            .rounding(Rounding::same(12.0))
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ui.horizontal(|ui| {
                            ui.label(RichText::new(&grade.code).size(16.0).strong().color(colors.on_surface));
                            ui.add_space(12.0);
                            ui.label(RichText::new(format!("${:.2}", grade.base_salary)).size(16.0).color(colors.primary));
                        });
                        ui.add_space(4.0);
                        let desc = grade.description.as_deref().unwrap_or("No description");
                        ui.label(RichText::new(desc).size(13.0).color(colors.on_surface_variant));
                        ui.add_space(2.0);
                        let emp_text = if employee_count == 1 { "employee" } else { "employees" };
                        ui.label(RichText::new(format!("{} {}", employee_count, emp_text)).size(12.0).color(colors.on_surface_variant));
                    });
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        if ui.add(edit_btn).clicked() {
                            ...
                            self.edit_dialog = Some(EditDialog::EditSalaryGrade { id: grade.id.clone() });
                        }
                    });
                });
            });
