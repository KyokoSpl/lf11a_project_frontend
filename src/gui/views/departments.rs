...
impl PersonnelApp {
    pub fn show_departments(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            ui.label(RichText::new("Departments").size(28.0).color(colors.on_surface).strong());
        ...
        ui.label(RichText::new("All Departments").size(16.0).color(colors.on_surface_variant));
        ui.add_space(12.0);
            ...
            Frame::none()
                ...
                .show(ui, |ui| {
                    ui.centered_and_justified(|ui| {
                        ui.label(RichText::new("No departments yet").color(colors.on_surface_variant));
                    });
                });
        }
        
        for dept in depts.iter() {
            let emp_count = emps.iter().filter(|e| e.department_id.as_ref() == Some(&dept.id)).count();
            self.render_dept_card(ui, &colors, dept, &emps, emp_count);
        }
    }
    
    fn render_dept_card(&mut self, ui: &mut Ui, colors: &Material3Colors, dept: &Department, emps: &[Employee], emp_count: usize) {
        Frame::none()
            .fill(colors.surface_variant)
            .stroke(Stroke::NONE)
            .rounding(Rounding::same(12.0))
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ui.label(RichText::new(&dept.name).size(16.0).strong().color(colors.on_surface));
                        ui.add_space(4.0);
                        let head_name = dept.head_id.as_ref()
                            .and_then(|id| emps.iter().find(|e| &e.id == id))
                            .map(|e| format!("{} {}", e.first_name, e.last_name))
                            .unwrap_or_else(|| "No head assigned".to_string());
                        ui.label(RichText::new(format!("👔 Head: {}", head_name)).size(13.0).color(colors.on_surface_variant));
                        ui.label(RichText::new(format!("👥 {} employee(s)", emp_count)).size(13.0).color(colors.on_surface_variant));
                    });
